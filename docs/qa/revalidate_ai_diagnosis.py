...
add(
    "Diagnosis Gaps",
    "DG-05",
    "user_id not auto-attached for history",
    "FAIL" if "user_id" in diag_api and "user_id" in diag_svc and "auth" in diag_ts.lower()
    else "PARTIAL",
    "Optional user_id on request; anonymous diagnoses won't appear in history",
    "P2",
)
...
add(
    "Valuation Remaining",
    "VR-02",
    "Next standalone /ai-valuation page",
    "FAIL" if not (ROOT / "apps/web/app/ai-valuation").exists() and not (ROOT / "apps/web/app/valuate").exists() else "PASS",
    "Still listing-button only on Next",
    "P1",
)
add(
    "Valuation Remaining",
