"""The car loan module, as a migration.

Companion to 0025, and the same reasoning: `schema_setup_batch7_car_loans.sql`
was applied by hand, so a deploy could not bring its own tables. This is that
file's DDL, statement for statement, including the eleven seeded lenders and
their rate cards — a lender directory with no lenders in it makes the feature
inert on a fresh database.

The seed inserts are ON CONFLICT DO NOTHING, so they never overwrite a rate an
admin has since corrected, and this whole migration is a no-op against the
production database that already ran the file.
...
def downgrade() -> None:
    op.execute("DROP TABLE IF EXISTS credit_checks;")
    op.execute("DROP TABLE IF EXISTS loan_offers;")
    op.execute("DROP TABLE IF EXISTS loan_applications;")
    op.execute("DROP TABLE IF EXISTS lender_rate_slabs;")
    op.execute("DROP TABLE IF EXISTS lending_partners;")
    op.execute("DROP TYPE IF EXISTS loan_application_status;")
    op.execute("DROP TYPE IF EXISTS loan_credit_source;")
    op.execute("DROP TYPE IF EXISTS loan_vehicle_condition;")
    op.execute("DROP TYPE IF EXISTS credit_band;")
    op.execute("DROP TYPE IF EXISTS lending_partner_type;")
    # loan_employment_type is left alone: 0025 does not create it, but a
    # database that ran the batch files may share it with other objects.
    op.execute("DROP TYPE IF EXISTS loan_employment_type;")
